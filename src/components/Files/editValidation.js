...
export default function EditFileValidations() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFinalValidations(response?.data?.Tables[0]?.columns);
        setrequestBody(response?.data?.APIMultiRequests[0]?.REQUESTBODY);
        if (response?.data?.APIMultiRequests[0]?.Headers.length > 0) {
          setinputParams(response.data.APIMultiRequests[0].Headers);
        }
        setvalfile(response?.data?.ApiInputFilePath);
      } catch (error) {
        setSnack({
  ...
  const ValidationCheck = async () => {
    const d1 = new Date();
    var data;
    if (file.connectionType === "Web App") {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <InnerHeader name={"Edit Data Quality Rule - Data Source"} />
      {response?.data?.result.ext === "json" ||
      response?.data?.result.ext === "xml" ? (
        <Paper>
          <CustomJsonTree
